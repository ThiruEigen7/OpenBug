...
══════════════════════════════════════════════════════════════════
Infineon Bug Detection Hackathon
Single-file Agentic AI Solution

LLM     : Google Gemini 2.5 Flash
Framework: LangChain ReAct Agent
MCP     : FastMCP Server (search_documents)
Output  : output.csv  +  audit.json
══════════════════════════════════════════════════════════════════
"""
...
# ─────────────────────────────────────────────
#  DATASET LOADER  (cached globally)
# ─────────────────────────────────────────────
...
def build_agent():
    ...
    print("[Agent] Initializing Gemini 2.5 Flash LLM...")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if confidence >= MIN_CONFIDENCE:
                ...
                print(f"[Pipeline] ⚠️  Low confidence ({confidence}), retrying...")
                time.sleep(1)
...
def run_pipeline():
    ...
    if os.path.exists(OUTPUT_CSV):
        ...
        print("⚠️  output.csv was not created — check errors above")
